...
from torch import nuclear_norm                         # system related library
...
Voltages = [3,3.5,4,4.5,5]      # V
num_measurements = 100          # number of times to run for loop
num_voltages = len(Voltages)
Acceleration_array = np.empty((num_measurements, num_voltages))



num_clk_pulse = 100
motor_direction = 'cw'          # 'ccw' or 'ccw'
...
## Need to write Accelerator data collection to fill Acceleration_array

Acceleration_mean = []
Acceleration_max = []
for i in range(0, len(Voltages)):
    Acceleration_mean.append(np.mean(Acceleration_array[i]))
    Acceleration_max.append(np.max(Acceleration_array))


plt.plot(Voltages, Acceleration_mean, label='Mean Acceleration')
plt.plot(Voltages, Acceleration_max, label='Max Acceleration')
plt.title('Mean & Max Acceleration vs Voltage')
plt.xlabel('Voltage')

...
def grab_convert(data_bit_name, loc_MSB=0x21, loc_LSB=0x20):
    ...
    if data_bit_name[-1] == 'M':
        ...
        LSB = dev.GetWireOutValue(loc_MSB)       # get msb temp register and shift 8 bits to the left
        MSB = dev.GetWireOutValue(loc_LSB)<<8          # get lsb temp register (may need to shift 3 to the right (>>3))
...
testread = "00110010001000000011001100000001"
dev.SetWireInValue(0x00, int(testread,2))
dev.SetWireInValue(0x01, 1)
dev.UpdateWireIns()
time.sleep(0.0001)
dev.UpdateWireOuts()
MSB=dev.GetWireOutValue(0x21)
LSB=dev.GetWireOutValue(0x20)
print(bin(MSB))
print(bin(LSB))

...
read_write = {
            ...
            '43_w':"0" + registers['43'] + str(format(5,'08b')) + zeros8 + write_bit, #originally 1 MSB
            '43_r':"0" + registers['43'] + str(format(5,'08b')) + zeros8 + read_bit, # OG 10
...
def buf_thread(dirinput):
    ...
    dev.SetWireInValue(0x06, 25)       # motor pulses
    dev.SetWireInValue(0x02, dirinput)  # direction of motor (0 ccw, 1 cw) 
    dev.SetWireInValue(0x08, 105)       # Motor dutycycle
...
def grab_convert(PCDATA):
    RW = 0
    dev.SetWireInValue(0x07,RW)                  # Turn R/W off
    dev.UpdateWireIns()
    RW = 1
    dev.SetWireInValue(0x07,RW)                 # Turn read on
    dev.SetWireInValue(0x0B,Addresses[PCDATA])               # Address
    dev.UpdateWireIns()  
    time.sleep(0.01)   
    dev.UpdateWireOuts()
...
#dev.SetWireInValue(0x07,2)                 # Turn write on
#dev.SetWireInValue(0x09,int(A_SAD + R,2))   # SAD + R: 51
#dev.SetWireInValue(0x0A,int(A_SAD + W,2))   # SAD + W: 50
#dev.SetWireInValue(0x0B,0x20)               # Write to CTRL REG
#dev.SetWireInValue(0x0C,0x27)         # Write TURN ON to CTRL
#dev.UpdateWireIns()
#dev.SetWireInValue(0x07,0)                  # Turn R/W off
#dev.UpdateWireIns()
#
## Write to 4A ctrl
#dev.SetWireInValue(0x07,2) 
#dev.SetWireInValue(0x09,int(A_SAD + W,2))
#dev.SetWireInValue(0x0A,int(A_SAD + R,2)) 
#dev.SetWireInValue(0x0B,0x23)             # Write to CRTL REG
#dev.SetWireInValue(0x0C,0x40)       # Write TURN ON to mag CTRL reg
#dev.UpdateWireIns()
...
## Mag sensor
#RW = 2
#dev.SetWireInValue(0x07,RW) 
#dev.SetWireInValue(0x09,int(M_SAD + W,2))
#dev.SetWireInValue(0x0A,int(M_SAD + R,2)) 
#dev.SetWireInValue(0x0B,0x02)             # Write to CRTL REG
#dev.SetWireInValue(0x0C,0x00)       # Write TURN ON to mag CTRL reg
#dev.UpdateWireIns()
#dev.SetWireInValue(0x07,0)
#dev.UpdateWireIns()
...
#    XM_data = (XHM<<8) + XLM
#    XM_data = twos_comp(XM_data) /1100 # Does not work without /1000?
#
#
#    PCDATA = 'YLM'
...
#    YM_data = (YHM<<8) + YLM
#    YM_data = twos_comp(YM_data) /1100
#
#
#    PCDATA = 'ZLM'
...
#    ZM_data = (ZHM<<8) + ZLM
#    ZM_data = twos_comp(ZM_data) /980

    # -----------------------------------------------------------------------------------------
    print('\nAccelerometer:\n\tX:{}\tY:{}\tZ:{}'.format(XA_data, YA_data, ZA_data)) # Print data
#    print('\nAccelerometer:\n\tX:{}\tY:{}\tZ:{}'.format(XM_data, YM_data, ZM_data)) # Print data
...
num_frames = 100
Intensity_5050 = np.zeros((num_frames +1, 10, 10)) # create a empty array the size of the numer of frames to fill with intensity data from pix 50,50
#Int_time = '1ms' # '10ms'
...
#save_image = np.zeros((pix1,pix2, num_frames))
diff_threshold = .75 # Difference in image threshold is 95% of maximum differnce in value (THIS NEEDS TO BE TUNED)
...
#    ret, th1 = cv2.threshold(frame_diff, diff_threshold*np.max(frame_diff), 255, 0)
#    x_avg = np.average(np.average(th1, axis=1))
...
#    Intensity_5050[counter] = image_F1[300:310,10:20]	# Get intensity of pixel 50, 50 (row 50, column 50) and add to array
#    save_image[:][:][counter] = image_F1
...
#frames = np.arange(0, num_frames,1) # create a array of frame numbers
#plt.plot(frames, np.average(Intensity_5050, axis=(1,2))[1:])
#plt.title('Intensity of 10x10 pixel square \n Standard Deviation = ' + str(np.std(np.average(Intensity_5050, axis=(1,2)))) + '\nS/N = ' + str(np.average(np.average(Intensity_5050))/np.std(Intensity_5050)))
#plt.ylabel('Intensity')
#plt.xlabel('Frame #')
#plt.tight_layout()
#plt.savefig("Intensity_pix5050_", dpi=300)
#
#plt.imshow(image_F1)

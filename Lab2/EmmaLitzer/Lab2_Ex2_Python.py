...
ConfigStatus=dev.ConfigureFPGA("lab2_example_frequency.bit");
...
counter_max = 100
counter_reset =0
frequency = int(10000000/10)
    
print("Control_var is " + str(int(control_var)) + "\nCounter_max is " + str(counter_max) + 
...
dev.SetWireInValue(0x01, frequency)
...
if ((control_var != 0) and (control_var !=1)):
    for i in range(0, 256):
        time.sleep(1/(200000000/frequency))   #match slow clk update speed              
        dev.UpdateWireOuts()
        counter = dev.GetWireOutValue(0x20)
        #counter_reset = counter
        #dev.SetWireInValue(0x02, counter_reset)
        #dev.UpdateWireIns()
